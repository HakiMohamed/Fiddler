# Guide d'Installation de Fiddler

Ce guide vous montre comment télécharger, installer et configurer Fiddler pour intercepter le trafic HTTP et HTTPS.

## Télécharger Fiddler

1. Allez sur le site officiel de Fiddler: [https://www.telerik.com/download/fiddler](https://www.telerik.com/download/fiddler).
2. Téléchargez la version de Fiddler pour votre système d'exploitation (Windows).
...
## Configuration de Fiddler pour Intercepter le Trafic HTTP et HTTPS

### Lancer Fiddler

1. Ouvrez Fiddler après l'installation.

### Configurer la capture HTTPS
...
class Handlers
{
	...
	static function getMACAddress(): String
	{
		...
		// Parcourir les interfaces pour trouver la première active non-loopback et non-virtuelle
		for (var i: int = 0; i < nics.Length; i++)
		{
			var nic = nics[i];
		...
		// Si aucune adresse MAC valide n'est trouvée, lever une exception
		if (String.IsNullOrEmpty(macAddress))
		{
    ...
    */
    ...
    static function OnBeforeRequest(oSession: Session) {
		...
		oSession.oRequest.headers.Add("mac-adresse", macAddress);
